...
@Slf4j
public class StateMachineNoRepositoryTest {
    ...
    public void happyPath1() {
        Entity e = new Entity(ID, VALUE);
        assertEquals(EntityStatus.OPENED, e.getState());
        
        stateMachine.onTransition(EntityEvent.START_WORKING, ID, e, null);
        assertEquals(EntityStatus.IN_PROGRESS, e.getState());
    }
    
    @Test
    public void nothing() {
        Entity e = new Entity(ID, VALUE);
        stateMachine.onTransition(EntityEvent.NOTHING, ID, e, null);
        assertEquals(EntityStatus.OPENED, e.getState());
        
    }
    
    @Test(expected=StateMachineConfigurationException.class)
    public void tryCallingTransitionWithRepository() {
        stateMachine.onTransition(EntityEvent.START_WORKING, "id");
    }
    
    @Test(expected=BadStateException.class)
    public void badStateException() {
        Entity e = new Entity(ID, VALUE);
        e.setState(EntityStatus.CLOSED);
        
        stateMachine.onTransition(EntityEvent.START_WORKING, ID, e, null);
    }
    
    @Test(expected=NoTargetStateException.class)
    public void NoTargetException() {
        Entity e = new Entity(ID, VALUE);
